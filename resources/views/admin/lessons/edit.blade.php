...
@section('title', 'Chỉnh Sửa Bài Học - Admin Fly High English')
...
<section class="bg-slate-900 text-white py-10">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <h1 class="text-2xl font-black">Chỉnh Sửa Bài Học: {{ $lesson->title }}</h1>
    </div>
</section>
        ...
        <form action="{{ route('admin.lessons.update', $lesson->id) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-sm space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div class="sm:col-span-2">
                    <label class="block text-xs font-bold text-slate-700 mb-1">Tên bài học *</label>
                ...
                <div class="flex items-center pt-6">
                    <label class="flex items-center gap-2 cursor-pointer text-sm font-bold text-slate-700">
                        ...
                        Cho phép xem thử bài học (Preview)
                    </label>
                </div>
            </div>
            ...
            <div class="p-5 rounded-2xl bg-slate-50 border border-slate-200 space-y-4">
                <h3 class="font-extrabold text-slate-900 text-sm">Tệp Bài Học Đính Kèm (Học viên tải về)</h3>
                
                <div>
                    <label class="block text-xs font-bold text-slate-700 mb-1">Thay đổi tệp bài học (Đường dẫn hiện tại: {{ $lesson->html_file_path ?? 'Chưa có file' }})</label>
                    <input type="file" name="html_file" class="w-full text-xs text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-extrabold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100">
                </div>
            </div>
