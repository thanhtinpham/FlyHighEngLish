...
@section('title', 'Chỉnh sửa thông báo - Admin')
...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-6">
    ...
    <div class="bg-white rounded-3xl border border-slate-100 shadow-xl p-8 sm:p-10 space-y-6">
        <div>
            <h1 class="text-2xl font-black text-slate-900">Chỉnh Sửa Thông Báo</h1>
            <p class="text-sm text-slate-500">Cập nhật nội dung bài đăng thông báo</p>
        </div>
        ...
        <form action="{{ route('admin.notifications.update', $notification) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" id="title" name="title" required value="{{ old('title', $notification->title) }}"
            ...
            <div>
                ...
                <textarea id="content" name="content" rows="6" required
                          class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-emerald-500 text-sm">{{ old('content', $notification->content) }}</textarea>
            </div>

            <div class="bg-amber-50 p-4 rounded-2xl border border-amber-200/80">
                <label class="flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" name="is_pinned" value="1" {{ old('is_pinned', $notification->is_pinned) ? 'checked' : '' }} class="w-5 h-5 rounded border-amber-300 text-amber-600 focus:ring-amber-500">
                    <div>
